...
ItemApp.config(['$locationProvider', '$routeProvider', function ($locationProvider, $routeProvider) {
  ...
  when('/list', {
    template: '<table id="titles" class="table table-striped">'+
		...
		'</thead>'+
			'<tbody>'+
				'<tr ng-repeat="item in ItemVariable | filter : ' + filter + '" class="events{{item.Event}} removed{{item.Removed}} unreleased{{item.Unreleased}} golden{{item.Golden}} exclusive{{item.Exclusive}} "><td>{{item.Title}}</td><td>{{item.Desc}}</td><td>{{item.Obtain}}<br/><span class="requirements">({{item.Char}} Required)</span></td><td class="str">STR +{{item.STR}}</td><td class="wil">WIL +{{item.WIL}}</td><td class="agi">AGI +{{item.AGI}}</td><td class="int">INT +{{item.INT}}</td><td class="hp">HP +{{item.HP}}</td></tr>'+
			'</tbody>'+
		'</table>',
